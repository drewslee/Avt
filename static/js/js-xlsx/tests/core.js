...
if (typeof process != 'undefined' && ((process || {}).env)) {
    ...
    if (process.env.FMTS) ex = process.env.FMTS.split(":").map(function (x) {
        return x[0] === "." ? x : "." + x;
    });
}
...
function test_file(x) {
    return ex.indexOf(x.slice(-5)) >= 0 || exp.indexOf(x.slice(-13)) >= 0 || ex.indexOf(x.slice(-4)) >= 0 || exp.indexOf(x.slice(-12)) >= 0;
}
...
function fixsheetname(x/*:string*/)/*:string*/ {
...
function fixcsv(x/*:string*/)/*:string*/ {
    return stripbom(x).replace(/\t/g, ",").replace(/#{255}/g, "").replace(/"/g, "").replace(/[\n\r]+/g, "\n").replace(/\n*$/, "");
}

function fixjson(x/*:string*/)/*:string*/ {
    return x.replace(/[\r\n]+$/, "");
}
...
function pathit(p, ext) {
    return ext.map(function (n) {
        return paths[p + n];
    });
}
...
function parsetest(x/*:string*/, wb/*:Workbook*/, full/*:boolean*/, ext/*:?string*/) {
    ...
    if (fs.existsSync(dir + '2011/' + x + '.xml'))
        describe(x + ext + '.xml from 2011', function () {
            it('should parse', function () {
                /*var wb = */
                X.readFile(dir + '2011/' + x + '.xml', opts);
            });
        });
    if (fs.existsSync(dir + '2013/' + x + '.xlsb'))
        describe(x + ext + '.xlsb from 2013', function () {
            it('should parse', function () {
                /*var wb = */
                X.readFile(dir + '2013/' + x + '.xlsb', opts);
            });
        });
...
function each_cell(ws, f) {
    if (Array.isArray(ws)) ws.forEach(function (row) {
        if (row) row.forEach(f);
    });
    else Object.keys(ws).forEach(function (addr) {
        if (addr[0] === "!" || !ws.hasOwnProperty(addr)) return;
        f(ws[addr]);
    });
}

function each_sheet(wb, f) {
...
describe('parse options', function () {
    ...
    describe('cell', function () {
        ...
        it('should generate cell styles when requested', function () {
            ...
            [paths.cssxlsx /*, paths.cssxlsb, paths.cssxls, paths.cssxml*/].forEach(function (p) {
                ...
                var found = false;
                each_sheet(wb, function (ws/*::, i*/) { /*:: void i; */
                    each_cell(ws, function (cell) {
        ...
        it('should handle stub cells', function () {
            MCPaths.forEach(function (p) {
                ...
                ofmt.forEach(function (f) {
                    if (f != "dbf") X.write(wb, {type: TYPE, bookType: f});
                });
            });
        });
    ...
    describe('book', function () {
        ...
        var NFVBA = ["nfxlsx", "nfxlsb", "nfxls"].map(function (n) {
            return paths[n];
        });
...
describe('input formats', function () {
    ...
    (typeof Uint8Array !== 'undefined' ? it : it.skip)('should read array', function () {
        artifax.forEach(function (p) {
            X.read(fs.readFileSync(p, 'binary').split("").map(function (x) {
                return x.charCodeAt(0);
            }), {type: 'array'});
        });
    });
    ((browser || typeof Buffer === 'undefined') ? it.skip : it)('should read Buffers', function () {
        artifax.forEach(function (p) {
            X.read(fs.readFileSync(p), {type: 'buffer'});
        });
    });
    (typeof Uint8Array !== 'undefined' ? it : it.skip)('should read ArrayBuffer / Uint8Array', function () {
        artifax.forEach(function (p) {
    ...
    it('should throw if format is unknown', function () {
        artifax.forEach(function (p) {
            assert.throws(function () {
                X.read(fs.readFileSync(p), {type: 'dafuq'});
            });
        });
    });

    var T = browser ? 'base64' : 'buffer';
    it('should default to "' + T + '" type', function () {
        artifax.forEach(function (p) {
            X.read(fs.readFileSync.apply(fs, browser ? [p, 'base64'] : [p]));
        });
    });
    if (!browser) it('should read files', function () {
        artifax.forEach(function (p) {
            X.readFile(p);
        });
    });
...
describe('output formats', function () {
    ...
    function RT(T) {
        ...
        fmts.forEach(function (fmt) {
            ...
            if (T == 'string' && !fmt[2]) return assert.throws(function () {
                X.write(wb, {type: T, bookType: fmt[0], WTF: 1});
            });
    ...
    it('should write JS strings', function () {
        RT('string');
    });
    if (typeof ArrayBuffer !== 'undefined' && (typeof process == 'undefined' || !process.version.match(/v0.12/))) it('should write array buffers', function () {
        RT('array');
    });
...
describe('API', function () {
    ...
    it('sheet_add_json', function () {
        ...
        X.utils.sheet_add_json(ws, [{A: 5, B: 6, C: 7}, {A: 6, B: 7, C: 8}, {A: 7, B: 8, C: 9}], {
            skipHeader: true,
            origin: {r: 1, c: 4},
            header: ["A", "B", "C"]
        });
        X.utils.sheet_add_json(ws, [{
            A: 4,
            B: 5,
            C: 6,
            D: 7,
...
function cmparr(x) {
    for (var i = 1; i < x.length; ++i) assert.deepEqual(x[0], x[i]);
}
...
describe('parse features', function () {
    describe('sheet visibility', function () {
        var wbs = [];
        var bef = (function () {
            wbs = SVPaths.map(function (p) {
                return X.read(fs.readFileSync(p), {type: TYPE});
            });
        });
        if (typeof before != 'undefined') before(bef);
        else it('before', bef);
    ...
    describe('should parse core properties and custom properties', function () {
        ...
        ['XLSX', 'XLSB', 'XLS', 'XML'].forEach(function (x, i) {
            it(x + ' should parse core properties', function () {
                coreprop(wbs[i].Props);
            });
            it(x + ' should parse custom properties', function () {
                custprop(wbs[i].Custprops);
            });
        });
        [
            ["asxls", "BIFF8", "\u2603"],
    ...
    describe('sheetRows', function () {
        it('should use original range if not set', function () {
            var opts = {type: TYPE};
            FSTPaths.map(function (p) {
                return X.read(fs.readFileSync(p), opts);
            }).forEach(function (wb) {
        ...
        it('should adjust range if set', function () {
            ...
            var wbs = FSTPaths.map(function (p) {
                return X.read(fs.readFileSync(p), opts);
            });
            /* TODO: XLS, XML, ODS */
        ...
        it('should not generate comment cells', function () {
            var opts = {type: TYPE, sheetRows: 10};
            var wbs = CSTPaths.map(function (p) {
                return X.read(fs.readFileSync(p), opts);
            });
            /* TODO: XLS, XML, ODS */
            wbs.slice(0, 2).forEach(function (wb) {
    ...
    describe('column properties', function () {
        ...
        var bef = (function () {
            X = require(modp);
            wbs = CWPaths.map(function (n) {
                return X.read(fs.readFileSync(n), {type: TYPE, cellStyles: true});
            });
    ...
    describe('row properties', function () {
        ...
        var bef = (function () {
            X = require(modp);
            wbs = RHPaths.map(function (p) {
                return X.read(fs.readFileSync(p), {type: TYPE, cellStyles: true});
            });
            /* */
            if (!ol) return;
            ols = OLPaths.map(function (p) {
                return X.read(fs.readFileSync(p), {type: TYPE, cellStyles: true});
            });
        });
        if (typeof before != 'undefined') before(bef);
    ...
    describe('should find hyperlinks', function () {
        ...
        var bef = (function () {
            X = require(modp);
            wb1 = HLPaths.map(function (p) {
                return X.read(fs.readFileSync(p), {type: TYPE, WTF: 1});
            });
            wb2 = ILPaths.map(function (p) {
                return X.read(fs.readFileSync(p), {type: TYPE, WTF: 1});
            });
        });
        if (typeof before != 'undefined') before(bef);
        ...
        ['xlsx', 'xlsb', 'xls', 'xml'].forEach(function (x, i) {
            it(x + " external", function () {
                hlink1(wb1[i].Sheets.Sheet1);
            });
        });
        ['xlsx', 'xlsb', 'xls', 'xml', 'ods'].forEach(function (x, i) {
            it(x + " internal", function () {
                hlink2(wb2[i].Sheets.Sheet1);
            });
        });
    });
    ...
    describe('page margins', function () {
        ...
        [
            ...
            it('should parse ' + t[0] + ' margin', function () {
                wbs.forEach(function (wb) {
...
describe('roundtrip features', function () {
    ...
    describe('should preserve merge cells', function () {
        ["xlsx", "xlsb", "xlml", "ods", "biff8"].forEach(function (f) {
            it(f, function () {
    ...
    describe('should preserve hyperlink', function () {
        [
            ...
            it(w[0] + " " + (w[2] ? "ex" : "in") + "ternal", function () {
                var wb = X.read(fs.readFileSync(w[1]), {type: TYPE, WTF: opts.WTF});
                var hlink = (w[2] ? hlink1 : hlink2);
    ...
    describe('should preserve row properties', function () {
        [
            ...
            it(w, function () {
                ...
                var wb1 = {SheetNames: ["Sheet1"], Sheets: {Sheet1: ws1}};
                var wb2 = X.read(X.write(wb1, {bookType: w, type: TYPE, cellStyles: true}), {
                    type: TYPE,
                    cellStyles: true
                });
                var ws2 = wb2.Sheets.Sheet1;
    ...
    describe('parse', function () {
        [
            ...
            it('should fail on ' + w[0], function () {
                assert.throws(function () {
                    X.read(fs.readFileSync(dir + w[1], 'binary'), {type: 'binary'});
                });
                assert.throws(function () {
...
function plaintext_test(wb, raw) {
    ...
    plaintext_val.forEach(function (x) {
        ...
        if (x.length == 1) {
            if (cell) {
                assert.equal(cell.t, 'z');
                assert(!cell.v);
            }
            return;
        }
        assert.equal(cell.v, tcval);
        assert.equal(cell.t, type);
    });
}

function make_html_str(idx) {
    return ["<table>",
...
describe('CSV', function () {
    describe('input', function () {
        ...
        it('should interpret values by default', function () {
            plaintext_test(X.read(csv_bstr, {type: "binary"}), false);
        });
        it('should generate strings if raw option is passed', function () {
            plaintext_test(X.read(csv_str, {type: "string", raw: true}), true);
        });
...
if (fs.existsSync('./test_files/dbf/d11.dbf')) describe('dbf', function () {
    ...
    var bef = (function () {
        wbs = wbs.map(function (x) {
            return [x[0], X.read(fs.readFileSync(x[1]), {type: TYPE})];
        });
    });
    if (typeof before != 'undefined') before(bef);
    else it('before', bef);
    it(wbs[1][0], function () {
        ...
        [
            ...
            assert.equal(get_cell(ws, r[0])[r[1]], r[2]);
        });
    });
});
var JSDOM = null;
// $FlowIgnore
var domtest = browser || (function () {
    try {
...
describe('HTML', function () {
    describe('input string', function () {
        it('should interpret values by default', function () {
            plaintext_test(X.read(html_bstr, {type: "binary"}), false);
        });
        it('should generate strings if raw option is passed', function () {
            plaintext_test(X.read(html_bstr, {type: "binary", raw: true}), true);
        });
        it('should handle "string" type', function () {
            plaintext_test(X.read(html_str, {type: "string"}), false);
        });
    ...
    (domtest ? describe : describe.skip)('input DOM', function () {
        it('should interpret values by default', function () {
            plaintext_test(X.utils.table_to_book(get_dom_element(html_str)), false);
        });
        it('should generate strings if raw option is passed', function () {
            plaintext_test(X.utils.table_to_book(get_dom_element(html_str), {raw: true}), true);
        });
        it('should handle newlines correctly', function () {
    ...
    describe('type override', function () {
        ...
        if (domtest) it('DOM', function () {
            chk(X.utils.table_to_sheet(get_dom_element(html)));
        });
    });
});
...
describe('js -> file -> js', function () {
    ...
    ofmt.forEach(function (f) {
        it(f, function () {
            ...
            var cb = function (cell) {
                eqcell(wb, newwb, 'Sheet1', cell);
            };
            ['A2', 'A3'].forEach(cb);
            /* int */
            ['A4', 'A5'].forEach(cb);
    ...
    password_files.forEach(function (x) {
        describe(x, function () {
            it('should throw with no password', function () {
                assert.throws(function () {
                    X.read(fs.readFileSync(dir + x), {type: TYPE});
                });
            });
...
if (!browser || typeof cptable !== 'undefined')
    describe('multiformat tests', function () {
        ...
        mft.forEach(function (x) {
            ...
            else x.split(/\s+/).forEach(function (w) {
                switch (w) {
                    case "no-csv":
                        csv = false;
                        break;
